// Tavernen-Minispiel Funktionen
static startTavernGame() {
    GameUI.clearActions();
    GameUI.addMessage("=== TRINKDUELL GEGEN ZWERGE ===", 'title');
    GameUI.addMessage("Willkommen in der Taverne 'Zum Trunkenen Drachen'!", 'npc');
    GameUI.addMessage("Ein grimmiger Zwerg fordert dich zu einem Trinkduell heraus.", 'npc');
    
    // Spielvariablen
    const playerDrinks = 0;
    const dwarfDrinks = 0;
    const playerTolerance = this.player.strength * 2 + this.player.level;
    const dwarfTolerance = 15 + Math.floor(Math.random() * 10);
    
    this.tavernGameState = {
        playerDrinks,
        dwarfDrinks,
        playerTolerance,
        dwarfTolerance,
